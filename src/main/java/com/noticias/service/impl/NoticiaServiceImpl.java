...
public class NoticiaServiceImpl implements NoticiaService {
    ...
    // ----------------------------------------------------------------
    // FN: PKG_COMENTARIOS.FN_CONTAR_COMENTARIOS_POR_NOTICIA
    // ----------------------------------------------------------------
    @Override
    public int contarComentariosPorNoticia(int idNoticia) {
        // SP: PKG_COMENTARIOS.FN_CONTAR_COMENTARIOS_POR_NOTICIA
        try (Connection conn = dataSource.getConnection();
             CallableStatement cs = conn.prepareCall(
                     "{? = call PKG_COMENTARIOS.FN_CONTAR_COMENTARIOS_POR_NOTICIA(?)}")) {
            cs.registerOutParameter(1, Types.NUMERIC);
            cs.setInt(2, idNoticia);
            cs.execute();
            return cs.getInt(1);
    ...
    // ----------------------------------------------------------------
    // FN: PKG_NOTICIAS.FN_CONTAR_NOTICIAS_POR_AUTOR
    // ----------------------------------------------------------------
    @Override
    public int contarNoticiasPorAutor(int idAutor) {
        // SP: PKG_NOTICIAS.FN_CONTAR_NOTICIAS_POR_AUTOR
        try (Connection conn = dataSource.getConnection();
             CallableStatement cs = conn.prepareCall(
                     "{? = call PKG_NOTICIAS.FN_CONTAR_NOTICIAS_POR_AUTOR(?)}")) {
            cs.registerOutParameter(1, Types.NUMERIC);
            cs.setInt(2, idAutor);
            cs.execute();
            return cs.getInt(1);
